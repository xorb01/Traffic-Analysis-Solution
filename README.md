...
### **2. 벡터 기반 정밀 카운팅**
CCW(벡터 교차) 알고리즘을 도입하여, P1(카운팅 라인 시작점), P2(카운팅 라인 끝점)과 차량의 이동 궤적 선분 P3(이전위치), P4(현재위치)의 교차 여부를 기하학적으로 판별.

교차 판정 로직
- 방향성 판정: 카운팅 라인 P1 -> P2를 기준으로 차량의 이전위치(P3)와 현재위치(P4)에 대해 각각 CCW연산을 수행.
- 교차 시점: 차량이 선을 통과하면 P3와 P4의 부호가 서로 반대가 됨.
...
### 3. 공간 분할 및 차선별 독립 분석
- 중앙 분리대를 기준으로 분석 영역을 좌/우로 분리하여, 각 차선에 독립적인 검지선과 카운팅 변수를 할당하여 교통량을 정밀하게 산출할 수 있도록 설계.
